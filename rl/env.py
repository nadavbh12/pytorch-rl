...
from utils.helpers import preprocessAtari, preprocessRle, rgb2gray, rgb2y, scale
...
class RleEnv(AtariEnv):
    def __init__(self, args, env_ind=0):
        super(AtariEnv, self).__init__(args, env_ind)

        assert self.env_type == "rle"
        try: import gym
        except ImportError as e: self.logger.warning("WARNING: gym not found")
        try: import gym_rle
        except ImportError as e: self.logger.warning("WARNING: gym-rle not found")
